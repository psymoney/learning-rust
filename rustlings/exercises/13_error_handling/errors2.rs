...
fn total_cost_variant_1(item_quantity: &str) -> Result<i32, ParseIntError> {
    let processing_fee = 1;
    let cost_per_item = 5;
...
fn total_cost_variant_2(item_quantity: &str) -> Result<i32, ParseIntError> {
    let processing_fee = 1;
    let cost_per_item = 5;

    item_quantity
        .parse::<i32>()
        .map(|qty: i32| qty * cost_per_item + processing_fee)
}
